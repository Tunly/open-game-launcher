...
export async function buildExportPayload(
  ...
  const modVersionIds = legacyMods.length === 0
    ? pluckIds(await deps.readRowsIn("mod_versions", "mod_id", [], warnings))
    : [];
  ...
  const resolvedModVersionIds = modVersions.length > 0
