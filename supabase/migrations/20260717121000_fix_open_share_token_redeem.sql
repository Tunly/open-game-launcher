-- The status hardening trigger made receiver_id immutable, but open share-token
-- redemption must atomically bind the previously-null receiver while accepting.
...
declare
  ...
  binds_open_receiver boolean :=
    old.receiver_id is null
    and new.receiver_id = actor_id
    and old.status = 'pending'
    and new.status = 'accepted';
begin
  if new.sender_id is distinct from old.sender_id
    or (new.receiver_id is distinct from old.receiver_id and not binds_open_receiver)
