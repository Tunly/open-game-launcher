...
function gameMatchesSearch(row: GameAchievementRow, query: string): boolean {
  ...
  return row.group.title.toLowerCase().includes(normalized)
...
function ArtworkPanel({ group }: { group: GameGroup }) {
  const iconCandidate = getGameIconCandidates(group.primaryGame)
    .map(getGameAssetUrl)
    .find(Boolean);
      ...
      <div className="grid gap-3 p-2 md:grid-cols-[280px_minmax(0,1fr)_36px]">
        ...
        <div className="min-w-0 py-1">
          <div className="flex min-w-0 items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-1.5">
                {isPerfect ? (
                  <span className="neo-copy border-2 border-black bg-[#e8c843] px-2 py-0.5 text-[9px] font-black uppercase shadow-[1px_1px_0_#171411]">
                    Perfect
                  </span>
                ) : null}
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-[90px_90px_minmax(160px,1fr)]">
            <div>
              <p className="neo-copy text-[10px] font-black uppercase text-[#5b403f]">Total Played</p>
              <p className="text-[12px] font-black text-[#171411]">{formatPlayTime(group.playtimeMinutes)}</p>
            </div>
            <div>
              <p className="neo-copy text-[10px] font-black uppercase text-[#5b403f]">Last Played</p>
              <p className="text-[12px] font-black text-[#171411]">
                {formatLastPlayed(group.lastPlayedAt ?? group.primaryGame.lastPlayedAt ?? group.primaryGame.lastPlayed)}
              </p>
            </div>
            <div className="min-w-0">
              <div className="mb-1 flex items-center justify-between gap-2">
                <p className="neo-copy text-[10px] font-black uppercase text-[#5b403f]">Achievements</p>
                <p className="neo-copy text-[10px] font-black uppercase text-[#171411]">{unlocked}/{total}</p>
              </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button className="neo-copy inline-flex h-8 items-center gap-2 border-2 border-black bg-[#171411] px-3 text-[10px] font-black uppercase text-[#fbf4e7] shadow-[2px_2px_0_#171411]" type="button">
              My Game Stats
              <ChevronDown className="h-3.5 w-3.5" />
            </button>
            <button className="neo-copy inline-flex h-8 items-center gap-2 border-2 border-black bg-[#fbf4e7] px-3 text-[10px] font-black uppercase text-[#171411] shadow-[2px_2px_0_#171411]" type="button">
              My Game Content
              <ChevronDown className="h-3.5 w-3.5" />
            </button>
          </div>
...
function StatCard({ label, value, tone }: { label: string; value: string | number; tone: "ink" | "teal" | "red" | "paper" }) {
  const toneClass = {
  ...
  return (
    <div className={`min-w-[92px] border-[3px] border-black p-3 shadow-[3px_3px_0_#171411] ${toneClass}`}>
...
export function AchievementsPage() {
  ...
  const tabCounts = useMemo(() => ({
    recent: rows.filter((row) => parseTime(row.group.lastPlayedAt) > Number.NEGATIVE_INFINITY).length,
    all: rows.length,
  ...
  const visibleRows = useMemo(() => {
    ...
    if (sortMode === "name") {
      ...
      next.sort((left, right) => right.completion - left.completion || right.unlocked - left.unlocked);
    }

    if (activeTab === "recent" && sortMode !== "name") {
      next.sort((left, right) => parseTime(right.group.lastPlayedAt) - parseTime(left.group.lastPlayedAt));
    }
    ...
    <section className="neo-dots space-y-5">
      <div className="mx-auto max-w-[980px] border-4 border-black bg-[#fbf4e7] shadow-[6px_6px_0_#171411]">
        <div className="flex flex-wrap items-center gap-4 border-b-4 border-black bg-[#171411] px-4 py-3 text-[#fbf4e7]">
          <div className="grid h-14 w-14 place-items-center border-[3px] border-black bg-[#e8c843] text-[#171411] shadow-[3px_3px_0_#000]">
          ...
          <div className="min-w-0">
            ...
            <h1 className="neo-title truncate text-4xl leading-none">
              Daniel <span className="text-xl text-[#e8c843]">/ Games</span>
            </h1>
          </div>
          ...
          <div className="grid min-h-[260px] place-items-center border-4 border-black bg-[#fbf4e7] p-6 text-center shadow-[5px_5px_0_#171411]">
            <div>
              ...
              <p className="neo-copy mt-3 text-[13px] font-black uppercase text-[#5b403f]">
                {rows.length === 0 ? "No achievement-enabled games found." : "No games match this filter."}
              </p>
            </div>
          </div>
        ) : (
