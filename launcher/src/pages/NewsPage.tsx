    ...
    <section className="flex flex-col gap-6 bg-[#fbf4e7] p-6">
      ...
      <div className="flex flex-col gap-4">
        {items.map((item) => (
          <div
            key={item.id}
            className="border-4 border-black bg-white p-4 shadow-[4px_4px_0_#171411]"
          >
            ...
            <div className="mt-2 flex gap-2">
              {item.tags.map((tag) => (
                <span key={tag} className="border border-black bg-[#8cf5e4] px-2 text-xs font-bold">
                  {tag}
                </span>
              ))}
            </div>
        ...
        {items.length === 0 && (
          <p className="text-sm text-gray-500">No news articles yet. Check back soon!</p>
        )}
      </div>
    </section>
  );
