...
const PROVIDERS: Array<{
  ...
  { key: "nexus", label: "Nexus Mods", short: "Nexus", mode: "hybrid" },
...
const FALLBACK_CATALOG: ModCatalogEntry[] = [
  ...
  {
    id: "demo-nexus-nxm",
    slug: "nexus-nxm-handoff",
    name: "Nexus NXM Handoff",
    author: "Nexus Mods",
    summary: "Uses official Nexus/Vortex or browser handoff when direct download is unavailable.",
    description: "A safe provider adapter placeholder for authenticated Nexus flows.",
    provider: "nexus",
    sourceUrl: "https://www.nexusmods.com/",
    externalId: "nexus",
    categories: ["Provider"],
    tags: ["api-key", "delegated"],
    status: "published",
    latestVersion: {
      id: "demo-nexus-nxm-v1",
      catalogModId: "demo-nexus-nxm",
      version: "external",
      fileSizeBytes: 0,
      installStrategy: "external",
      isLatest: true,
...
export function ModsPage() {
  ...
  const activeCount = useModInstallStore((state) => state.activeCount());
  const delegatedCount = useModInstallStore((state) => state.delegatedCount());
  const completedCount = useModInstallStore((state) => state.completedCount());
  const totalProgress = useModInstallStore((state) => state.totalProgress());
  ...
  const [secretProvider, setSecretProvider] = useState<ModProvider>("nexus");
  ...
  useEffect(() => {
    ...
    const unlistenPromise = listen<ModInstallQueueItem>(
      "mod_install_progress",
      (event) => {
        if (!active) return;
  ...
  async function handleUninstallMod(mod: InstalledModInfo) {
    try {
      ...
      setInstalledMods((current) =>
        current.filter((entry) => entry.installId !== mod.installId),
      );
    } catch (err) {
      setError(getErrorMessage(err));
    }
    ...
    <section className="space-y-5">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            ...
            <p className="neo-copy mt-3 max-w-[760px] text-xs font-black uppercase text-[#5b403f]">
              Steam Workshop // Nexus // mod.io // CurseForge // local archive
            </p>
          </div>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        <label className="relative flex-1">
          ...
          <input
            className="neo-copy h-14 w-full border-4 border-black bg-[#fff9ed] pl-11 pr-4 text-[13px] font-black uppercase outline-none shadow-[4px_4px_0_#171411]"
      ...
      <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="space-y-4">
          <Panel title="Game Target" icon={<PackagePlus className="h-4 w-4" />}>
            <label className="neo-copy block text-[10px] font-black uppercase text-[#5b403f]">
              Local Library
              <select
                className="mt-2 h-11 w-full border-4 border-black bg-[#fff9ed] px-2 text-[12px] font-black text-[#171411] outline-none shadow-[3px_3px_0_#171411]"
              ...
              >
                {games.length === 0 ? (
                  <option value="">No installed games detected</option>
                ) : null}
                {games.map((game) => (
        ...
        <div className="space-y-4">
          ...
          <div className={`grid gap-4 ${showAdvanced ? "2xl:grid-cols-[minmax(0,1fr)_360px]" : ""}`}>
            ...
            {showAdvanced && (
              <Panel title="Manual Install" icon={<FileArchive className="h-4 w-4" />}>
                <div className="space-y-3">
                  ...
                  />
                  <TextInput
                    label={manualProvider === "local_archive" || manualProvider === "local_folder" ? "Local path" : "Source URL"}
                    placeholder={
                      manualProvider === "local_archive" || manualProvider === "local_folder"
                        ? "C:/Mods/package.zip"
                        : "https://.../download.zip"
  ...
  return (
    <section className="border-4 border-black bg-[#f5eedf] shadow-[5px_5px_0_#171411]">
      <div className="flex items-center gap-2 border-b-4 border-black bg-[#171411] px-3 py-2 text-[#fff9ed]">
        {icon}
        <h2 className="neo-copy text-[11px] font-black uppercase tracking-normal">
          {title}
        </h2>
      </div>
    ...
    <article className="grid gap-3 border-2 border-black bg-[#fff9ed] p-3 shadow-[2px_2px_0_#171411] lg:grid-cols-[1fr_120px_auto] lg:items-center">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <span className={`neo-copy border-2 border-black px-2 py-0.5 text-[9px] font-black uppercase ${statusTone(item.status)}`}>
            {item.status}
          </span>
        ...
        <h3 className="mt-1 truncate text-lg font-black uppercase leading-none">
          {item.title}
        </h3>
        {item.delegatedUrl ? (
          <a
  ...
  return (
    <article className="flex min-h-[190px] flex-col border-4 border-black bg-[#fff9ed] shadow-[4px_4px_0_#171411]">
      ...
      <div className="flex flex-1 flex-col p-3">
        ...
        <button
          className="neo-copy mt-auto flex h-10 items-center justify-center gap-2 border-2 border-black bg-[#b7102a] px-3 text-[10px] font-black uppercase text-white shadow-[2px_2px_0_#171411] disabled:cursor-not-allowed disabled:opacity-50 transition-transform active:translate-x-[2px] active:translate-y-[2px] active:shadow-none"
          disabled={disabled}
          type="button"
          onClick={onInstall}
        >
...
function InstalledModRow({
  ...
  return (
    <article className={`grid gap-3 border-2 border-black p-3 shadow-[2px_2px_0_#171411] lg:grid-cols-[1fr_auto_auto] lg:items-center ${mod.enabled ? "bg-[#fff9ed]" : "bg-[#efe6d4]"}`}>
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <span className={`neo-copy border-2 border-black px-2 py-0.5 text-[9px] font-black uppercase ${mod.enabled ? "bg-[#007166] text-white" : "bg-[#171411] text-white"}`}>
...
function providerArt(provider: ModProvider) {
  ...
  if (provider === "nexus") return "library-source-art library-source-art-ubisoft";
