...
export function StorePage() {
  ...
  const [ownedIds] = useState<Set<string>>(() => new Set(readStringArray(ownedKey)));
  ...
  async function buyNow(gameId: string) {
    ...
    try {
      const { data, error } = await getSupabaseClient().functions.invoke("stripe-create-checkout", {
        body: {
          line_items: [
            {
              price_data: {
          ...
          <div className="relative m-4 flex min-h-[330px] items-center border-l-4 border-[#c20b2f] bg-black/62 p-5 sm:m-6 sm:min-h-[280px] sm:p-9">
            <div className="max-w-[590px]">
              ...
              <div className="mt-8 grid gap-3 sm:flex sm:flex-wrap sm:items-center sm:gap-5">
                <button
                  className="neo-copy flex h-12 items-center justify-center gap-3 border-2 border-black bg-[#c20b2f] px-5 text-xs font-bold text-[#fffaf0] uppercase shadow-[4px_4px_0_#171411] sm:px-7 disabled:opacity-50"
                  type="button"
