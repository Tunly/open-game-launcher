...
describe("getLogoPlacementStyle", () => {
  ...
  it("clamps the percent values to allowed ranges", () => {
    const style = getLogoPlacementStyle(
      makeGame({ logoWidthPercent: 5, logoHeightPercent: 99 }),
    );
...
describe("getPlatformBannerClass", () => {
  it("returns a class per known platform", () => {
    expect(getPlatformBannerClass(makeGame({ id: "steam-1" }))).toBe(
      "steam-game-banner-hero",
    );
  });

  it("returns the matching banner class for known sources", () => {
    expect(getPlatformBannerClass(makeGame({ id: "xbox-1" }))).toBe(
      "xbox-game-banner-hero",
    );
    expect(getPlatformBannerClass(makeGame({ id: "epic-1" }))).toBe(
      "epic-game-banner-hero",
    );
  });
});