...
import type { Friendship, Profile, ProfileComment, ProfileShowcase, ProfileTheme, UserActivity, UserBadge, UserSocialLink } from "../../types/profile";
...
export function toHardware(row: UnknownRecord | null): import("../../types/profile").UserHardware | null {
  if (!row) return null;
  return {
    userId: rowString(row, "user_id"),
    ...
    visibility: rowString(row, "visibility", "friends_only") as import("../../types/profile").UserHardware["visibility"],
    createdAt: rowString(row, "created_at"),
    updatedAt: rowString(row, "updated_at"),
  };
}
