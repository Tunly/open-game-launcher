...
export async function getOverlaySettings(): Promise<OverlaySettings | null> {
  ...
  const { data, error } = await client.from("overlay_settings").select("*").eq("user_id", user.id).single();
...
export async function saveOverlaySettings(): Promise<OverlaySettings | null> {
  const client = getSupabaseClient(); if (!client) return null;
  const { data: { user } } = await client.auth.getUser(); if (!user) return null;
  const { data, error } = await client.from("overlay_settings").upsert({ user_id: user.id }).select("*").single();
