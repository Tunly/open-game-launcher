...
export async function listPublishedProducts(): Promise<StoreProduct[]> {
  ...
  const { data, error } = await client.from("store_products")
    .select(PRODUCT_SELECT).eq("status", "published").order("created_at", { ascending: false });
...
export async function getStoreProduct(slug: string): Promise<StoreProduct | null> {
  ...
  const { data, error } = await client.from("store_products").select(PRODUCT_SELECT).eq("slug", slug).single();
...
export async function listDeveloperProducts(): Promise<StoreProduct[]> {
  ...
  if (!user) return [];
  const { data, error } = await client.from("store_products")
    .select(PRODUCT_SELECT).eq("developer_id", user.id).order("updated_at", { ascending: false });
  if (error) throw new Error(error.message);
  return (data ?? []) as unknown as unknown as StoreProduct[];
}

export async function createStoreProduct(title: string, slug: string): Promise<StoreProduct | null> {
  const client = getSupabaseClient(); if (!client) return null;
...
export async function getCartItems(): Promise<StoreCartItem[]> {
  ...
  const { data, error } = await client.from("store_cart_items")
    .select(CART_SELECT).eq("user_id", user.id);
  if (error) throw new Error(error.message);
...
export async function addToCart(productId: string, quantity = 1): Promise<void> {
  const client = getSupabaseClient(); if (!client) return;
  const { data: { user } } = await client.auth.getUser(); if (!user) return;
  const { error } = await client.from("store_cart_items").upsert(
    { user_id: user.id, product_id: productId, quantity }, { onConflict: "user_id,product_id" }
  );
  if (error) throw new Error(error.message);
}
...
export async function removeFromCart(productId: string): Promise<void> {
  const client = getSupabaseClient(); if (!client) return;
  const { data: { user } } = await client.auth.getUser(); if (!user) return;
  const { error } = await client.from("store_cart_items")
    .delete().eq("user_id", user.id).eq("product_id", productId);
  if (error) throw new Error(error.message);
}

// --- Builds ---
export async function getLatestBuild(productId: string, platform: string): Promise<StoreBuild | null> {
  const client = getSupabaseClient(); if (!client) return null;
  const { data, error } = await client.from("store_builds")
    .select(BUILD_SELECT).eq("product_id", productId).eq("platform", platform).eq("is_latest", true).single();
  if (error) return null;
...
export async function getMyLicenses(): Promise<StoreLicense[]> {
  const client = getSupabaseClient(); if (!client) return [];
  const { data: { user } } = await client.auth.getUser(); if (!user) return [];
  const { data, error } = await client.from("store_licenses")
    .select(LICENSE_SELECT).eq("user_id", user.id).eq("is_revoked", false);
  if (error) throw new Error(error.message);
...
export async function submitDeveloperApplication(
  studioName: string, website: string | null, description: string | null
): Promise<DeveloperApplication | null> {
  const client = getSupabaseClient(); if (!client) return null;
  const { data: { user } } = await client.auth.getUser(); if (!user) return null;
  const { data, error } = await client.from("developer_applications")
