...
export function selectDelegatedModInstallCount(state: ModInstallState): number {
  return state.items.filter((item) => item.status === "delegated").length;
}

export function selectCompletedModInstallCount(state: ModInstallState): number {
  return state.items.filter((item) => item.status === "completed").length;
}

export function selectModInstallTotalProgress(state: ModInstallState): number {
...
function normalizeModInstallItem(item: ModInstallQueueItem): ModInstallQueueItem {
...
function sortQueueItems(left: ModInstallQueueItem, right: ModInstallQueueItem) {
  return (
    statusRank(left.status) - statusRank(right.status) || right.lastUpdatedAt - left.lastUpdatedAt
  );
}
