...
describe("useDownloadStore", () => {
  ...
  it("merges updates for an existing gameId", () => {
    act(() => {
      ...
      useDownloadStore.getState().upsertItem(makeItem({ gameId: "game-1", progress: 50, status: "downloading" }));
    });
  ...
  it("clamps NaN/Infinity progress to 0", () => {
    ...
    act(() => {
      useDownloadStore.getState().upsertItem(makeItem({ gameId: "g2", progress: Number.POSITIVE_INFINITY }));
    });
    // clampProgress treats anything non-finite (NaN, +/-Infinity) as 0.
    expect(useDownloadStore.getState().items.find((item) => item.gameId === "g2")!.progress).toBe(0);
  });

  it("canCancel and canPause are cleared for terminal items", () => {
    act(() => {
      useDownloadStore.getState().upsertItem(
        makeItem({ canCancel: true, canPause: true, status: "downloading" }),
      );
    });
    act(() => {
      useDownloadStore.getState().upsertItem(
        makeItem({ canCancel: true, canPause: true, status: "completed" }),
      );
    });
...
describe("downloadStore derived counts", () => {
  ...
  it("totalProgress averages live items and falls back to 100 when only completed exist", () => {
    ...
    expect(useDownloadStore.getState().totalProgress()).toBe(50);

    act(() => {
      useDownloadStore.getState().setItems([makeItem({ gameId: "c", status: "completed" })]);
    });
    expect(useDownloadStore.getState().totalProgress()).toBe(100);
  });
});
