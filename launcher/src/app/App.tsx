...
const FpsHudPageLazy = lazy(() => import("../pages/FpsHudPage").then((m) => ({ default: m.FpsHudPage })));
    ...
    <div className="fixed right-4 top-4 z-[9999] flex flex-col gap-2">
      {popups.map((p, i) => (
        <div
          ...
          className="flex w-72 items-center gap-3 border-[3px] border-[#171411] bg-[#fbf8ef] px-3 py-2 shadow-[4px_4px_0_#1f1c0f] neo-dots"
        >
          ...
          <div className="min-w-0">
            <div className="text-[9px] font-black uppercase text-[#b7102a] neo-copy">Erfolg freigeschaltet</div>
            <div className="truncate text-[12px] font-bold text-[#171411]">{p.achievement_name}</div>
            <div className="truncate text-[10px] font-bold text-[#655f58] neo-copy">{p.game_title}</div>
          </div>
          <button
        ...
        <Suspense
          fallback={
            <div className="grid h-screen w-screen place-items-center bg-black/70">
              <div className="border-[3px] border-[#171411] bg-[#171411]/80 px-4 py-3 neo-copy text-sm font-black uppercase text-[#fff9ed] shadow-[4px_4px_0_#000]">Lade Overlay...</div>
            </div>
          }
        >
          <OverlayPage />
