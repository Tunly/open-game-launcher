...
export function StoreGameCard({
  ...
  return (
    <article className="overflow-hidden border-4 border-black bg-[#f5eedf] shadow-[5px_5px_0_#171411]">
      ...
      >
        {game.id === "deep-signal" ? (
          <div className="absolute bottom-8 left-12 h-16 w-36 -skew-x-12 rounded-[45%] border-4 border-[#171411] bg-[#ece8de] shadow-[26px_20px_0_rgba(23,20,17,0.16)]">
            ...
            <div className="absolute right-6 -bottom-4 h-8 w-8 rounded-full border-4 border-[#171411] bg-[#f5eedf]" />
            <div className="absolute top-4 left-10 h-5 w-16 border-2 border-[#171411]" />
          </div>
        ) : null}
        {game.id === "haven-forge" ? (
      ...
      <div className="p-4">
        <h3 className="text-[clamp(1.35rem,7vw,1.5rem)] leading-none font-black text-[#171411] uppercase">
          {game.title}
        </h3>
        <p className="neo-copy mt-2 text-xs font-bold text-[#171411] uppercase">
          {game.description}
        </p>
        <div className="mt-3 flex flex-wrap gap-2">
          {(game.genres ?? [game.tagLine]).slice(0, 3).map((genre) => (
            <span
              key={genre}
              className="neo-copy border-2 border-black bg-[#efe3cf] px-2 py-1 text-[9px] font-black tracking-[0.1em] text-[#171411] uppercase"
            >
              {genre}
            </span>
          ))}
          {game.discountPercent ? (
            <span className="neo-copy border-2 border-black bg-[#b7102a] px-2 py-1 text-[9px] font-black tracking-[0.1em] text-white uppercase">
              -{game.discountPercent}%
            </span>
          ) : null}
        </div>
        ...
        <div className="flex items-end justify-between gap-4">
          <div>
            {game.originalPrice && game.originalPrice > game.price ? (
              <p className="neo-copy text-[10px] font-black text-[#655f58] uppercase line-through">
        ...
        <button
          className="neo-copy mt-4 h-10 w-full border-2 border-black bg-[#171411] px-3 text-[10px] font-black tracking-[0.12em] text-[#fff9ed] uppercase shadow-[3px_3px_0_#171411] transition hover:-translate-y-0.5 disabled:opacity-60"
          disabled={isAdded}
          type="button"
        ...
        <button
          className="neo-copy mt-2 h-9 w-full border-2 border-black bg-[#fff9ed] px-3 text-[10px] font-black tracking-[0.12em] text-[#171411] uppercase shadow-[3px_3px_0_#171411] transition hover:-translate-y-0.5"
          type="button"
        ...
        {priceAlert !== null ? (
          <p className="neo-copy mt-3 border-2 border-black bg-[#f2c14e] p-2 text-[9px] font-black tracking-[0.1em] text-[#171411] uppercase">
            Alert below{" "}
            {new Intl.NumberFormat("en-US", {
              currency: "EUR",
              style: "currency",
        ...
        {isPriceAlertEditorOpen && priceAlert === null ? (
          ...
          >
            <input
              className="neo-copy min-w-0 flex-1 border-2 border-black bg-[#fff9ed] px-2 text-[10px] font-black text-[#171411] uppercase outline-none"
              inputMode="decimal"
            ...
            />
            <button
              className="neo-copy border-2 border-black bg-[#f2c14e] px-3 text-[10px] font-black text-[#171411] uppercase shadow-[2px_2px_0_#171411]"
              type="submit"
            >
              Save
            </button>
