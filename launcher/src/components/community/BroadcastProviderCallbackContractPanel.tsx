import { Database, RadioTower, RotateCcw, ShieldCheck, Webhook } from "lucide-react";

import type {
  BroadcastProviderCallbackContract,
  BroadcastProviderCallbackContractItem,
  BroadcastProviderCallbackContractStatus,
} from "../../lib/broadcast-provider-callback-contract";

export function BroadcastProviderCallbackContractPanel({
  contract,
}: {
  contract: BroadcastProviderCallbackContract;
}) {
  return (
    <section
      aria-label="Broadcasting provider callback contract"
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3 border-b-4 border-black pb-3">
        <div>
          ...
          <h2 className="neo-title mt-1 flex items-center gap-2 text-3xl text-[#171411] uppercase">
            <Webhook aria-hidden="true" className="h-8 w-8" />
      ...
      <div className="mt-4 grid items-start gap-3 lg:grid-cols-[230px_minmax(0,1fr)_300px]">
        <div className="grid gap-3">
          <CallbackStat label="Contract review" value={`${contract.reviewCount}`} />
          <CallbackStat label="Blocked lanes" value={`${contract.blockedCount}`} />
        </div>

        <div className="grid items-start gap-3 md:grid-cols-2">
          {contract.items.map((item) => (
            <CallbackContractCard item={item} key={item.id} />
          ))}
        </div>
...
function CallbackStat({ label, value }: { label: string; value: string }) {
  return (
...
function CallbackContractCard({ item }: { item: BroadcastProviderCallbackContractItem }) {
  const Icon =
    item.id === "replay-duplicate-fixture" || item.id === "replay-runner"
      ? RotateCcw
      : item.id === "supabase-callback-write"
        ? Database
        : item.id === "provider-delivery"
          ? RadioTower
          : Webhook;
...
function statusClass(status: BroadcastProviderCallbackContractStatus) {
