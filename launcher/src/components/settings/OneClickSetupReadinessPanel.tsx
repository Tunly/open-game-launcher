      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.35fr)] lg:items-start">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span
              className={`neo-copy border-2 border-black px-2 py-1 text-[9px] font-black tracking-[0.12em] uppercase shadow-[2px_2px_0_#171411] ${toneClass}`}
            >
              {readiness.blockedCount > 0
                ? "blocked"
                : readiness.warningCount > 0
...
function SetupStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="border-2 border-black bg-[#fff9ed] p-2 shadow-[2px_2px_0_#171411]">
      ...
      <strong className="neo-title mt-1 block text-2xl text-[#171411] uppercase">{value}</strong>
    </div>
  );
}
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            <h3 className="neo-copy truncate text-[11px] font-black text-[#171411] uppercase">
              {step.label}
            </h3>
