  ...
  return (
    <div className="border-2 border-black bg-[#f6edd8] p-3 shadow-[2px_2px_0_#171411]">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b-2 border-black pb-2">
        ...
        <button
          className="neo-copy h-7 shrink-0 border-2 border-black bg-[#f56c2d] px-3 text-[9px] font-black uppercase text-white shadow-[1px_1px_0_#171411] disabled:opacity-50"
          disabled={running}
          type="button"
      ...
      {suggestions.slice(0, 10).map((sug) => (
        ...
        >
          <p className="neo-copy break-words text-[10px] font-bold text-[#171411]">{sug.reason}</p>
      ...
      {unmatchedLinks.length > 0 && (
        <div className="mt-3 space-y-2">
          ...
          {unmatchedLinks.slice(0, 15).map((link) => (
            <div
              key={link.id}
              className="flex flex-wrap items-start justify-between gap-2 border-2 border-black bg-[#f6edd8] p-2 shadow-[1px_1px_0_#171411]"
            >
              <div className="min-w-0 flex-1">
                <div className="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1">
                  <span className="neo-copy min-w-0 break-words text-[10px] font-bold text-[#171411]">
                    {link.platformFriendName ?? link.platformFriendId}
                  </span>
                  <span className="neo-copy shrink-0 text-[9px] uppercase text-[#55504a]">
                    ({link.platform})
                  </span>
                </div>
                {link.mergeGroupId && (
                  <div className="mt-2 flex min-w-0 flex-wrap items-center gap-1 border border-black bg-[#fff9ed] px-2 py-1 shadow-[1px_1px_0_#171411]">
                    <span className="neo-copy shrink-0 bg-[#087d6d] px-1.5 py-0.5 text-[8px] font-black uppercase text-white">
                      Merge group staged
                    </span>
                    <span className="neo-copy min-w-0 break-words text-[8px] font-bold uppercase text-[#5b403f]">
                      {link.mergeGroupId}
                    </span>
                  </div>
                )}
              ...
              <button
                className="neo-copy h-6 shrink-0 border border-black bg-[#efe6d4] px-2 text-[8px] font-bold uppercase text-[#55504a]"
                type="button"
