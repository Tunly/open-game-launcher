...
import type {
  ...
  AppWideThemeReadinessGate,
  AppWideThemeReadinessStatus,
} from "../../lib/app-wide-theme-readiness";

export function AppWideThemeReadinessPanel({ readiness }: { readiness: AppWideThemeReadiness }) {
  const tone =
    readiness.blockedCount > 0 ? "blocked" : readiness.warningCount > 0 ? "warning" : "ready";

  return (
    <section
      aria-label="App-wide Theme/Skin readiness"
    ...
    >
      ...
      <div className="mt-4 grid gap-3">
        <div className="border-2 border-black bg-[#efe3cf] p-3 shadow-[3px_3px_0_#171411]">
          <p className="neo-copy text-[10px] font-black uppercase text-[#5f574d]">Skin Score</p>
        ...
        <div className="grid gap-3">
          {readiness.gates.map((gate) => (
            <AppWideThemeGateCard gate={gate} key={gate.id} />
          ))}
        </div>
...
function AppWideThemeGateCard({ gate }: { gate: AppWideThemeReadinessGate }) {
  return (
    <article
...
function statusClass(status: AppWideThemeReadinessStatus) {
