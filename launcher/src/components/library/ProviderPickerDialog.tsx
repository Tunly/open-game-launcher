...
export function ProviderPickerDialog({ state, onClose, onSelect }: ProviderPickerDialogProps) {
  ...
  return (
    <ModalDialog
      labelledBy={titleId}
      backdropClassName="fixed inset-0 z-[85] grid place-items-center bg-[#171411]/90 bg-[radial-gradient(circle,rgba(255,249,237,0.14)_1px,transparent_1px)] bg-[length:10px_10px] px-4"
      panelClassName="max-h-[calc(100vh-2rem)] w-full max-w-[560px] overflow-y-auto border-4 border-black bg-[#fbf4e7] shadow-[8px_8px_0_#171411]"
      initialFocusSelector="[data-provider-option]"
      onDismiss={onClose}
    >
      <div className="flex items-center justify-between gap-3 border-b-4 border-black bg-[#b7102a] px-4 py-3 text-white">
        <div className="min-w-0">
          ...
          <h2 id={titleId} className="neo-title truncate text-2xl leading-none uppercase">
            {state.title}
          </h2>
        </div>
        <button
            ...
            >
              ...
              <span
                className={`flex h-10 items-center gap-2 border-2 border-black px-3 text-[11px] font-black uppercase shadow-[2px_2px_0_#171411] ${
                  isPlayMode ? "bg-[#007166] text-white" : "bg-[#b7102a] text-white"
                }`}
              >
