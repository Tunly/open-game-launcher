...
fn should_delegate_provider(input: &ModInstallRequest) -> bool {
    match input.provider {
        ...
        ModProvider::Nexus => {
            let url = input.source_url.as_deref().unwrap_or_default();
            url.starts_with("nxm://")
                || !provider_secret_exists(ModProvider::Nexus)
                || !looks_like_download_url(url)
        }
...
fn delegated_url_for_provider(input: &ModInstallRequest) -> Option<String> {
    ...
    match input.provider {
        ...
        ModProvider::Nexus | ModProvider::Modio | ModProvider::Curseforge => {
