...
pub async fn start_download(
    ...
    tokio::spawn(async move {
        ...
        let is_steam_download = game_id_clone.starts_with("steam-");

        // Create install folder and write manifest.
        ...
        // Update installed games cache for non-Steam games
        // (Steam games are detected automatically via their appmanifest)
        if !is_steam_download {
            let cache_path = dirs::data_local_dir()
                .or_else(dirs::data_dir)
                .map(|d| d.join("open-game-launcher").join("installed-games.json"));
        ...
        // We do this for both Steam and non-Steam games.
